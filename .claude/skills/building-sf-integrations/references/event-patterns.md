...
## Testing Platform Events and CDC

Use `Test.getEventBus()` to control the test event bus. The test bus is **separate** from the production bus — CometD and Pub/Sub API subscribers do not fire in tests; only Apex triggers and Flows do.

### Key Test APIs

| Method | Purpose |
|--------|---------|
| `Test.getEventBus().deliver()` | Deliver all queued test events to Apex/Flow subscribers now |
| `Test.getEventBus().fail()` | Simulate publish failure — invokes `onFailure()` callback |
| `Test.enableChangeDataCapture()` | Enable CDC event generation in the test context |
| `Test.startTest()` / `Test.stopTest()` | Events are also delivered automatically after `stopTest()` |

Limits in test context:
- Max **500 events per test method** across all `EventBus.publish()` calls.
- Replay IDs reset to 0; `EventBusSubscriber.Position` and `.Tip` reset to 0.
- After sandbox refresh or org migration, pre-activity Replay IDs in production are invalid.
...
static void testOrderEventPublishing() {
    Order testOrder = TestDataFactory.createOrder(50000);

    Test.startTest();
    OrderEventPublisher.publishOrderCompleted(new List<Order>{ testOrder });
    Test.stopTest(); // events delivered here

    // Verify via SaveResult from a direct publish
    Order_Completed__e evt = new Order_Completed__e(
        Order_Id__c = testOrder.Id, Total_Amount__c = testOrder.TotalAmount
    );
    List<Database.SaveResult> results = EventBus.publish(
        new List<Order_Completed__e>{ evt }
    );
...
static void testOrderCompletedSubscriber() {
    ...
    Order_Completed__e event = new Order_Completed__e(
        ...
        Customer_Id__c = acct.Id,
        Fulfillment_Status__c = 'Pending'
    );

    Test.startTest();
    EventBus.publish(event);
    Test.getEventBus().deliver(); // deliver immediately without stopTest
    Test.stopTest();

    List<Task> tasks = [SELECT Id, Priority FROM Task WHERE Subject LIKE '%ORD-001%'];
    Assert.areEqual(1, tasks.size());
    Assert.areEqual('Normal', tasks[0].Priority);
}
```
...
static void testAccountChangeEventTrigger() {
    Test.enableChangeDataCapture(); // required to generate CDC events in tests
    ...
    // Assert on side-effects your CDC trigger produces
}
```
...
- [Platform Events Limits](./platform-events-limits.md) - Edition-specific allocations and API limits
