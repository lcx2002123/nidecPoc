...
Hard rules for all Apex test classes. Applied automatically — no override without explicit justification.

### Never

| # | Rule |
|---|------|
| N1 | **Never use `@isTest(SeeAllData=true)`.** Breaks test isolation; tests must not depend on org-specific data. |
| N2 | **Never write a test without assertions.** A test with no assertion proves nothing and gives false confidence. |
| N3 | **Never hardcode Record IDs.** IDs differ across orgs; insert records and query them back. |
| N4 | **Never test only SOQL.** SOQL-only coverage inflates numbers without validating business logic. |
| N5 | **Never use `System.debug` as a substitute for assertions.** Debug output is invisible to the test framework. |
| N6 | **Never combine multiple unrelated scenarios in one test method.** Separate positive, negative, and bulk scenarios into distinct methods. |
...
| A2 | Always use `TestDataFactory` for record creation — never inline DML in `@TestSetup`. |
| A3 | Always assert both positive AND negative (exception) cases. |
| A4 | Always include a bulk test with 251+ records to cross the 200-record trigger batch boundary. |
| A5 | Always wrap code under test with `Test.startTest()` / `Test.stopTest()`. |
| A6 | Always use descriptive test method names: `should[Result]_When[Scenario]` or `testMethod_scenario_expectedResult`. |
| A7 | Always add `@testFor` annotation on test classes (required for `RunRelevantTests` in Spring '26 CI/CD). |

### Coverage Targets

| Deployment target | Minimum | Recommended |
|---|---|---|
| Production org-wide | 75% | 85%+ per class |
...
| [references/integration-test-patterns.md](references/integration-test-patterns.md) | Multi-step workflow tests, Flow integration, Platform Event subscriber verification, System.runAs, performance assertions |
