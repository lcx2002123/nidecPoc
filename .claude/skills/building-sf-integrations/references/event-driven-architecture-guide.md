...
Use the Saga pattern when a business operation spans multiple independent steps, each in its own transaction. Each step publishes a completion event; if any step fails, a compensation event triggers rollback logic in prior steps.
...
If any step fails:
  →  PE: Order_Compensation__e  (triggers rollback in all prior steps)
```

Implementation outline:

```apex
// Step 2 subscriber: reserve inventory and advance the saga
trigger InventoryReservationTrigger on Order_Created__e (after insert) {
    for (Order_Created__e evt : Trigger.new) {
        Boolean reserved = InventoryService.reserve(evt.Order_Id__c, evt.Quantity__c);

        if (reserved) {
            EventBus.publish(new Inventory_Reserved__e(Order_Id__c = evt.Order_Id__c));
        } else {
            // Trigger compensation — downstream handlers must be idempotent
            EventBus.publish(new Order_Compensation__e(
                Order_Id__c = evt.Order_Id__c,
                Reason__c = 'Insufficient inventory'
            ));
        }
...
Design rules for Saga:
- Every step handler **must be idempotent** — retries will re-execute it.
- Compensation events must be handled by **all prior steps**, not just the failing one.
- Keep event payloads small — pass IDs and essential context; let consumers re-query for full records.
- Saga state (which steps completed) should be tracked in a custom object, not inferred from events.
