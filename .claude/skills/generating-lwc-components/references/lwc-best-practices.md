...
|--------|--------------|-----------|
| `lightning/uiRecordApi` | `getRecord`, `getRecords`, `getRecordCreateDefaults` | `createRecord`, `updateRecord`, `deleteRecord`, `notifyRecordUpdateAvailable`, `getFieldValue`, `getFieldDisplayValue` |
...
| `lightning/uiRelatedListApi` | `getRelatedListRecords`, `getRelatedListCount`, `getRelatedListInfo`, `getRelatedListsInfo` | — |
...
> `lightning/uiListApi` (`getListUi`) is deprecated — use `lightning/uiListsApi` instead.
...
| Shadow DOM access | Slotted elements: `this.querySelector()`. Shadow elements: `this.template.querySelector()`. |
| `slotchange` event | Fires when direct children of a slot change. Does NOT fire for nested child changes. |
...
| No ID selectors | IDs are transformed at render time — never use `querySelector('#id')` for slotted elements. |
