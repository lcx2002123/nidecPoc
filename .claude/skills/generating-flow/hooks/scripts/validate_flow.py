...
class EnhancedFlowValidator:
    ...
    def _validate_security(self) -> Dict:
        ...
        if api_version < 66.0:
            ...
            advisory.append({
                'category': 'Governance',
                'message': f'API version {api_version} is outdated (current: 66.0)',
    ...
    def generate_report(self) -> str:
        ...
        if not results['critical_issues']:
            report.append("")
            report.append("📦 NEXT STEP - Use deploying-metadata skill (REQUIRED):")
            report.append("─"*70)
            ...
            report.append("   ✅  ALWAYS use deploying-metadata skill for consistent deployment")
            report.append("═"*70)
